...
import (
	dpfm_api_input_reader "data-platform-api-invoice-document-doc-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-invoice-document-doc-reads-rmq-kube/DPFM_API_Output_Formatter"
	"fmt"
...
func (c *DPFMAPICaller) HeaderDoc(
	...
	if input.HeaderDoc.InvoiceDocument != nil {
		where = fmt.Sprintf("%s\nAND InvoiceDocument = %d", where, *input.HeaderDoc.InvoiceDocument)
	}
	...
	groupBy := "\nGROUP BY InvoiceDocument, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    InvoiceDocument, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
...
func (c *DPFMAPICaller) ItemDoc(
	...
	if input.HeaderDoc.ItemDoc.InvoiceDocumentItem != nil {
		where = fmt.Sprintf("%s\nAND InvoiceDocumentItem = %d", where, *input.HeaderDoc.ItemDoc.InvoiceDocumentItem)
	}
	...
	groupBy := "\nGROUP BY InvoiceDocument, InvoiceDocumentItem, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    InvoiceDocument, InvoiceDocumentItem, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_invoice_document_item_doc_data
		` + where + groupBy + `;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
